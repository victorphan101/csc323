...
class forgot:
    ...
    def POST(self):
        ...
        if user in user_dic:
            ...
            # base64.b64encode(displayToken.encode('utf-8'))
            print(token)
...
def generate_token(user):
    ...
    mt = MT19937.MT19937(0x31337)
    token = mt.extract_number()
    # for i in range(7):
    #     token += ":" + str(mt.extract_number())
    displayToken = str(token)

    # print("token is ", token)
    mtorig = random.Random(seed)
    # [mtorig.getrandbits(32) for _ in range(1234)]
    # outputList = [mtorig.getrandbits(32) for _ in range(1000)]
    outputList = []
    for i in range(1337):
        r1 = mtorig.getrandbits(32)
        outputList.append(r1)


    # undo mt
    mtb = MT19937.MTUntwist()
    randomGen = random.Random(0x31337)
    # move internal state forward
    [randomGen.getrandbits(32) for _ in range(624)]
    # the actual leak of 1000 values
    n = [randomGen.getrandbits(32) for _ in range(624)]
    print(outputList)
    recover = mtb.go(n)

    print("recover is ", recover.getrandbits(32))

    for i in range(7):
        displayToken += ":" + str(mt.extract_number())
    return base64.b64encode(displayToken.encode('utf-8'))
